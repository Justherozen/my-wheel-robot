...
class Config():
    ...
    def __init__(self):
        ...
        self.speed_cost_gain = 10 #lower = faster
        self.obs_cost_gain = 1.5 #lower z= fearless
    ...
    def updatepredicttime(self):
        if((math.sqrt((self.x - self.goalX)**2 + (self.y - self.goalY)**2))<0.8):
            self.predict_time=1
        else:
            self.predict_time=3
...
def motion(x, u, dt):
    ...
    if u[1]!=0:
        x[0] += u[0]/u[1]*(math.sin(x[2]+u[1]*dt)-math.sin(x[2]))
        x[1] += u[0]/u[1]*(math.cos(x[2])-math.cos(x[2]+u[1]*dt))
    else:
...
def dwa_control(x, u, config, ob):
    ...
    while(i<config.predict_time/config.dt):
        pose = PoseStamped()
        pose.header.seq = i
        pose.header.stamp = rospy.Time(0)
        pose.header.frame_id = 'map'
        pose.pose.position.x = config.x+i*(u[0])/(u[1])*(math.sin(x[2]+u[1]*config.dt)-math.sin(x[2]))
        pose.pose.position.y = config.y+i*(u[0])/(u[1])*(math.cos(x[2])-math.cos(x[2]+u[1]*config.dt))
        print("draw")
